/*예제 1: 요소 내용 변경하기
id가 'title'인 <h1> 요소의 내용을 'DOM 조작 성공!'으로 변경하시오.*/
const title = document.querySelector('#title')
title.innerHTML = 'DOM 조작 성공!'
// VS
document.querySelector('#title').innerHTML = 'DOM 조작'

/*예제 2: 사용자 입력값 가져와서 표시하기
id가 'usernameInput'인 <input>에 이름을 입력하고 '확인' 버튼(id: 'submitBtn')을 클릭하면, id가 'greeting'인 <p> 태그에 "안녕하세요, [입력된이름]님!"이라고 표시하시오.*/
function 문자입력(){
    const 입력마크업 = document.querySelector('#usernameInput')
    const 입력값 = 입력마크업.value;
    document.querySelector('#greeting').innerHTML = 입력값
}

/*예제 3: 요소 스타일 동적으로 변경하기
'스타일 변경' 버튼(id: 'styleBtn')을 클릭하면, id가 'colorBox'인 <div>의 배경색을 'skyblue'로, 글자색을 'white'로 변경하시오.*/
function 스타일(){
    const 문제3 = document.querySelector('#colorBox')
    문제3.style = "background-color: skyblue; color: white;"
    // VS  css속성명 (카멜표기법
    문제3.style.backgroundColor = 'red'
    문제3.style.color = 'white'  
}

/*예제 4: CSS 클래스 토글하기 (다크 모드 예시)
'테마 변경' 버튼(id: 'themeBtn')을 클릭할 때마다 <body> 태그에 'dark-mode' 클래스를 추가하거나 제거(토글)하시오. */
function 테마변경(){
    const body = document.querySelector('body')
    body.classList.toggle('dark-mode')  // 클래스 추가 혹은 제거 (있으면 제거)
    // .classList : 클랙스 목록 반환
    console.log(body)  // class가 버튼을 누를떄 마다 바뀌는 모습을 확인 가능
}

/*예제 5: 특정 HTML 요소 제거하기
'박스 삭제' 버튼(id: 'removeBtn')을 클릭하면, id가 'targetBox'인 <div> 요소를 문서에서 제거하시오. */
function 삭제(){
    const 삭제위치 = document.querySelector('#targetbox')
    // console.log(삭제위치)
    삭제위치.remove() // .remove() 삭제 
}

/*예제 6: 여러 요소에 동일한 작업 반복하기
클래스가 'item'인 모든 <p> 요소의 글자색을 'green'으로, 글자 두께를 'bold'로 변경하시오. */
const 글자색변경 = document.querySelectorAll('.item')  // 여러개 가져오기 [ ] 배열 형태로 가져옴
for(let i = 0; i<= 글자색변경.length-1; i++){
    글자색변경[i].style = 'color: green; font-weight: bold;'
}

/*예제 7: 이미지 소스(src) 변경하기 , https://placehold.co/ 
'이미지 변경' 버튼(id: 'changeImgBtn')을 클릭하면, id가 'mainImage'인 <img> 요소의 src 속성을 'https://placehold.co/600x400/red/white'로 변경하시오. */
function 이미지변경(){
    const 이미지 = document.querySelector('#mainImage')
    이미지.src = 'https://placehold.co/600x400/red/white'
}

/* 예제 8 : select 에서 선택한 option값 console 출력하기. */
function func8(){  // onchange 선택의 따른 변경
    let 옵션 = document.querySelector('#foodSelect')
    let 옵션값 = 옵션.value
    console.log(옵션값)
}

/* 예제 9 : input 에서 입력받은 값을 전역 배열에 저장하고 배열 상태를 출력하기. */
let 전역 = [ ]
function func9(){
    let 입력 = document.querySelector('#itemInput')
    let 입력값 = 입력.value
    전역.push(입력값)
    console.log(전역)

    document.querySelector('#arrayStatus').innerHTML = 전역
}

